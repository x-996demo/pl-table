    ...
    setColumnRenders(column) {
      ...
      if (column.type === 'expand') {
        ...
        column.renderCell = (h, data) => {
          ...
          let row = data.row
          const plTreeTable = this.$parent
          if (plTreeTable.rowKey && column.treeNode && plTreeTable.useVirtual) {
              return (<div { ...props }>
                     <div class="cell--tree-node"
                          style={[{paddingLeft: (this.owner.getTreeLevel(row) * plTreeTable.treeOpts.indent) + 'px'}]}>
                         {
                             row[plTreeTable.treeOpts.children] && row[plTreeTable.treeOpts.children].length > 0
                               ? <div class={[{'tree--btn-wrapper-show': this.owner.isTreeExpandByRow(row)}, 'tree--btn-wrapper']}
