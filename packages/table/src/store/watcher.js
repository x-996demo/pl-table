...
export default Vue.extend({
  computed: {
    selecting() {
      let ret = [];
      let s1 = _.findIndex(this.states.data, x => x.$ix === this.states.start);
      let s2 = _.findIndex(this.states.data, x => x.$ix === this.states.end);
      let start = _.min([s1, s2]);
      let end = _.max([s1, s2]);
      for (let ix = start; ix <= end; ix++) {
        if (this.states.data[ix] !== undefined) {
          ret.push(this.states.data[ix].$ix);
        }
      }
      return ret;
    }
  ...
  methods: {
    ...
    // mouseup cuihovah备注
    dragmouseup(selectable, item) {
      if (this.states.mousedown === true) {
        _.each(this.selecting, m => {
          if (this.states.selected.indexOf(m) >= 0) {
            this.states.selected = _.filter(this.states.selected, x => x !== m);
          } else {
            if (selectable(item) === true) {
              this.states.selected.push(m);
            }
          }
        });
        this.states.start = -1;
      ...
      let ret = [];
      _.each(this.states.selected, ix => {
        let data = _.find(this.states.data, x => x.$ix === ix);
        if (data !== undefined) {
          ret.push(data);
        }
      });
      this.table.$emit('drag-change', ret);
    },
    dragmousedown(index) {
      this.states.mousedown = true;
      this.states.start = index;
      this.states.end = index;
    },
    dragmousemove(index) {
      if (this.states.mousedown === true) {
        this.states.end = index;
    ...
    dragall() {
      this.states.selected = _.pluck(this.states.data, '$ix');
    }
  }
});
